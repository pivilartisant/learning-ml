...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # nn.Conv2d(in_channels, out_channels, kernel_size) creates a 2D convolutional layer.
        # kernel
        # Kernel (also called a **filter**) is a small matrix used in convolution operations to scan over input data (like images). At each position, it computes a weighted sum (dot product) of the input values it overlaps with. This helps detect specific features such as edges, textures, or patterns.
